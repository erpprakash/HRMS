...
import { KeyboardAvoidingView, SafeAreaView, StyleSheet, View, Text, TouchableOpacity, Alert, Modal, ScrollView, Image, ImageBackground } from "react-native";
...
import Colour from  '../Constants/Colour';
import ColorPicker from '../Constants/ColourPicker';
...
export default function Home({ Token }) {
    const { userDetail, handleLogout,handleTheme,Theme } = useContext(DataContext);
    // const Colour = getColors(Theme);
...
console.log("Color44",Theme)
    ...
    const handleColorSelect = async(color) => {
        console.log(color,"colormanoj")
        await AsyncStorage.setItem('theme', color);
        handleTheme(color)
        setSharedState(color)
        ...
        const getUserValue = async () => {
            try {
              ...
              if (value !== null) {
                // Value was found, parse it if needed
